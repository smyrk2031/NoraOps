...
- **バックアップ復元**: 確認ダイアログに「戻す前には戻れない」警告を追加
- **保存タブ**: 公開動線を一本化（「保存と同時に公開」チェックを廃止、起動ファイルをメイン画面へ、版番号は公開モーダル内のみ）

## [0.26.0] — 2026-07-07

### Added

- **クラウド保存**: 非公開バックアップのローカルスナップショット（直近2件・ワンクリック復元）
- **保存タブ UX**: 接続・保存先・最終バックアップの状態カード、非エンジニア向け文言（「クラウドにバックアップ」「みんなに公開する」）
- **バックアップ履歴パネル**: 旧 git log 表示からローカルスナップショット一覧に刷新

### Changed

- 保存成功時にこの PC へのバックアップ記録を案内
- 詳細設定（公開状態・サムネ・保存対象など）は折りたたみに移動
