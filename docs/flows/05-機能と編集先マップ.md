...
| ホーム UI | `homePanel.js`, `noraops-home.html` | — |
...
| uv / git 配布 | `toolInstaller.js` ↔ `routers/tools.py`, `data/tools/` |
...
## 6. まだフロー図が無いが有用なトピック（将来）

| トピック | 既存の正本 |
|----------|------------|
| PyPI ミラー閉ループ | pypiserver 連携設定書 · [06 §4.3](./06-データのつながりと保管.md) 一部 |
| ワークスペース AppData 移行 | ワークスペース設計改定.md · [06 §5](./06-データのつながりと保管.md) |
| AI コンシェルジュ | `routers/concierge.py`, `routers/ai.py` |

必要になったら `docs/flows/` に `06-...md` を追加してください。
