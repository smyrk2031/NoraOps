...
> **現行バージョン**: 拡張 **v0.28.1** · バックエンド **v0.28.1**（2026-07 時点。矛盾時はコードが勝ち）
...
| タブ | 主な利用者 | できること |
|------|------------|------------|
| **Setting** | 全員 | サーバー URL・プロキシ・アカウント（OTP）・ツール（uv）状態・セキュリティガード概要・**拡張バージョン表示**・接続失敗時の案内バナー |
...
| 機能 | 説明 | 主なファイル |
|------|------|--------------|
| カタログ検索 | 公開リポジトリを検索（**既定: 自分が使える** = `?scope=mine`、要ログイン） | `catalogClient.js`, `portal_api.py` |
